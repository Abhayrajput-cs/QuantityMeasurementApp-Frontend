...
export class DashboardPageComponent {
  ...
  readonly selectedType = signal<MeasurementType>('LENGTH');
  ...
  firstValue = 1;
  firstUnit = unitOptions('LENGTH')[0].value;
  secondValue = 1;
  secondUnit = unitOptions('LENGTH')[1].value;
  targetUnit = unitOptions('LENGTH')[2].value;
  ...
  async runOperation(): Promise<void> {
    ...
    try {
      const measurementType = this.selectedType();
  ...
  // ✅ Round properly (no floating garbage)
  const value = Number(response.resultValue);
  this.conversionValue = Number.isInteger(value)
    ? value.toString()
...
response.resultValue === 1 ? '✅ Values are EQUAL' : '❌ Values are NOT EQUAL'
  ...
  return;
      ...
      const response = await this.quantityApi.calculate(
        this.selectedCalculatorOperation,
        payload
      );

      this.calculationSummary = `Ans = ${response.resultValue} `;
